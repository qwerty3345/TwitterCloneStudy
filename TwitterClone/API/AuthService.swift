...
// TODO: shared 객체를 생성해 싱글턴 방식으로 사용하는 것과, static func 로 사용하는 것의 차이는?? 메모리 구조 측면에서.
...
// TODO: AuthCredentials 이라는 이름보다, 어차피 모델 역할을 하는 아이니까, User 라는 모델 객체를 쓰는게 낫지 않을까? -> 근데 그러면 User는 profileImageUrl을 가져야 하는데 얘는 UIImage를 직접 가지니 그게 다르겠구나..
...
struct AuthService {
    ...
    /// 회원가입 (FirebaseAuth), Credentials는 Wrapper Class임.
